...
import type { SeriesCategory, SeriesStatus } from '@prisma/client'
...
export class SeriesService {
  ...
  async getSeriesRecommendations(userId: string, limit = 5): Promise<SeriesRecommendation[]> {
    ...
    const userSeriesIds = await prisma.article.findMany({
      ...
      distinct: ['seriesId']
    }).then(results => results.map(r => r.seriesId).filter((id): id is string => id !== null))
    ...
    const recommendations: SeriesRecommendation[] = availableSeries.map(series => {
      ...
      const estimatedReadTime = (series as any).articles.reduce(
        (total: any, article: any) => total + article.readTime,
        0
      )
      ...
      return {
        ...
        articlesCount: (series as any)._count.articles,
        estimatedReadTime,
        matchScore,
