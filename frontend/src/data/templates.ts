...
export const getTemplatesByCategory = (category: string) => {
  return mockTemplates.filter(template => template.category === category && template.isActive)
}
...
export const convertToLegacyTemplate = (template: Template): any => {
  return {
    ...
    settings: {
      textLength: template.sections.length > 2 ? 'long' : template.sections.length > 1 ? 'medium' : 'short',
      allowVideos: template.mediaSettings.video.enabled,
      maxVideos: template.mediaSettings.video.maxVideos,
      videoTypes: template.mediaSettings.video.platforms,
      allowImages: template.mediaSettings.gallery.enabled || template.mediaSettings.featuredImage.required,
      maxImages: template.mediaSettings.gallery.maxImages + (template.mediaSettings.featuredImage.required ? 1 : 0),
      imageLayout: template.mediaSettings.gallery.enabled ? 'gallery' : 'inline',
      allowDownloads: template.mediaSettings.documents.enabled,
      ...
      styling: {
        ...
        fontSize: template.style.typography.fontSize,
        spacing: template.style.spacing.padding,
        colors: {
          primary: template.style.colors.primary,
          secondary: template.style.colors.secondary,
          text: template.style.colors.text
        }
      }
    }
  }
...
export const articleTemplates = getActiveTemplates().map(convertToLegacyTemplate)